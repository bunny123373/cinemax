    ...
    try {
      ...
      if (type !== "all") {
        items = items.filter((r) =>
          type === "movie" ? r.type === "movie" : r.type === "tv"
        );
      }
      setResults(items);
    } catch {
      ...
      <div className="max-w-[1800px] mx-auto px-4 md:px-8">
        <form onSubmit={handleSubmit} className="mb-8">
          <div className="relative max-w-2xl mx-auto">
            <input
              ...
              className="w-full pl-12 pr-4 py-3 rounded-lg bg-[#12121a] border border-[#2a2a3a] text-white placeholder-[#8e8ea0] focus:outline-none focus:border-[#f5c542]/50 transition-colors text-lg"
            />
          ...
          {["all", "movie", "tv"].map((t) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                typeFilter === t || (t === "tv" && typeFilter === "series")
                  ? "bg-[#f5c542] text-[#0a0a0f]"
                  : "bg-[#12121a] text-[#8e8ea0] hover:text-white border border-[#2a2a3a]"
              }`}
            >
              {t === "all" ? null : t === "movie" ? <Film className="w-4 h-4" /> : <Tv className="w-4 h-4" />}
        ...
        {!loading && searched && results.length === 0 && (
          <div className="text-center py-20">
            <p className="text-[#8e8ea0] text-lg">No results found for &quot;{query}&quot;</p>
          </div>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2">
                    ...
                    <p className="text-xs text-[#8e8ea0]">{item.year}</p>
                  </div>
                </Link>
              );
            })}
        ...
        {!loading && !searched && (
          <div className="text-center py-20">
            ...
            <p className="text-[#8e8ea0]">Search for your favorite movies and series</p>
          </div>
        )}
      </div>
    </div>
